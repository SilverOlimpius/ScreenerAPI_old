...
haveShare_list = DATASHARE['GrPetYear'].tolist()
LOST_allTicket = [ticket for ticket in allTicket if ticket not in haveShare_list]

start_time = datetime.datetime.now()

DATASHARES = []
list_share = ['PHM', 'DHI', 'LGIH','AAPL','MSFT','IBP','GTN','PATK','REGN','WGO','CAMT','CTRA','AMN','LCII','THO','MBUU']

#allTicket = list_share
for share in LOST_allTicket:
    print(share)
    #try:
    ...
    #    df.to_csv('AddDATASHARESexcept.csv', index=False)
    #    print('ExceptSAVE',share)
...
df.to_csv('AddDATASHARESfull.csv',index=False)

sorted_data = df.sort_values(by='bvps', ascending=False)
print(sorted_data)
